...
void TryLearningSpells(CCharEntity* PChar, CMobEntity* PMob) {
	...
	// populate PBlueMages
        if (PChar->PParty != nullptr)
        {
            ...
            {
                if ((PChar->PParty->members[i]->GetMJob() == JOB_BLU || ((map_config.dual_main_job) && (PChar->PParty->members[i]->GetSJob() == JOB_BLU))) &&
                    PChar->PParty->members[i]->objtype == TYPE_PC)
                {
	...
	for (size_t i = 0; i < PBlueMages.size(); i++) {
		...
		for (size_t spell = 0; spell < PLearnableSpells.size(); spell++) {
            ...
            if (playerSkillLvl + 30 >= skillLvlForSpell)
            {
                int chanceToLearn = 35 + PBlueMage->getMod(Mod::BLUE_LEARN_CHANCE) + playerSkillLvl - skillLvlForSpell;
                chanceToLearn = std::clamp(chanceToLearn, 10, 70);
