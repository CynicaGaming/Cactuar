    ...
    void FishingAction(CCharEntity* PChar, FISHACTION action, uint16 stamina, uint32 special)
    {
        ...
        case FISHACTION_CHECK:
        {
            ...
            else if (fishingArea != nullptr && response != nullptr && response->caught && response->catchtype > 0 && response->catchid > 0)
            {
                ...
                // ?!?!?!?!? fish is on the line here... going off wiggo capture
                PChar->updatemask |= UPDATE_HP;
                //ShowDebug("Sending CFishingPacket data to client, params %u %u %u %u %u %u %u %u %u\n", response->stamina, response->regen, response->response, response->attackdmg, response->delay, response->heal, response->timelimit, response->hooksense, response->special);
