...
bool CStatusEffectContainer::ApplyCorsairEffect(CStatusEffect* PStatusEffect, uint8 maxRolls, uint8 bustDuration)
{
    ...
    for (auto&& PEffect : m_StatusEffectSet)
    {
            ...
            PEffect->GetStatusID() == EFFECT_BUST)//is a cor effect
        {
            ...
            { // same type, double up or reapply regardless if a more powerfull roll exists
                ...
                else
                {
                    if (PEffect->GetSubID() == m_POwner->id)
                    {
                        ...
                        //
                        // I.E. in 2011 they added the functionality of not giving the bust penalty if you have an 11 roll present, here we should always get
                        // the bust if (!CheckForElevenRoll())
                        //{
                            uint16 duration = 300;
                            ...
                            CStatusEffect* bustEffect =
                                new CStatusEffect(EFFECT_BUST, EFFECT_BUST, power, 0, duration, PStatusEffect->GetTier(), PStatusEffect->GetStatusID());
