...
CMenuMeritPacket::CMenuMeritPacket(CCharEntity* PChar)
{
    ...
    if (PChar->GetMJob() == JOB_BLU && PChar->GetMLevel() || PChar->GetSJob() == JOB_BLU && PChar->GetSLevel() > 74)
    ...
    bits += (PChar->jobs.job[PChar->GetMJob() || PChar->GetSJob()] >= 75 && charutils::hasKeyItem(PChar, 606)) << 13; // is 75 and has limit breaker KI
    bits += ((PChar->jobs.job[PChar->GetMJob() || PChar->GetSJob()] >= PChar->jobs.genkai &&
              PChar->jobs.exp[PChar->GetMJob() || PChar->GetSJob()] == charutils::GetExpNEXTLevel(PChar->jobs.job[PChar->GetMJob() || PChar->GetSJob()]) - 1) ||
             PChar->MeritMode)
            << 14;                                        // my exp is capped
