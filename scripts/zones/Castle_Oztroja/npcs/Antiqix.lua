...
function onTrade(player, npc, trade)
   ...
   if player:hasKeyItem(tpz.ki.VIAL_OF_SHROUDED_SAND) == true then
      ...
      elseif (gil == 0) then
         ...
         -- Currency Shop
         elseif (count == 1 and trade:hasItemQty(1450, 1)) then -- T. Whiteshell (1449)
            buying = true
            exchange = {1, 1449}
            quantity = 100
...
function onEventFinish(player, csid, option)
   ...
   elseif (csid == 56) then -- Trading 100 Hundreds for Ten thousand
      if (player:getFreeSlotsCount() == 0) then
         player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, 1451)
      else
         player:tradeComplete()
         player:addItem(1451)
   ...
   elseif (csid == 58) then -- Trading Ten thousand for 100 Hundreds
      if (player:getFreeSlotsCount() <= 1) then
         ...
         player:messageSpecial(ID.text.ITEMS_OBTAINED, 1450, CURRENCY_EXCHANGE_RATE)
      end
   end
end
