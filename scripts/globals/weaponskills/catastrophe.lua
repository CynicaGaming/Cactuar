...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
    ...
    params.str_wsc = 0.6 params.dex_wsc = 0.0 params.vit_wsc = 0.0 params.agi_wsc = 0.0 params.int_wsc = 0.4 params.mnd_wsc = 0.0 params.chr_wsc = 0.0
    ...
    if not target:isUndead() then
        local drain = math.floor(damage * (math.random(75,85)/100))
        player:addHP(drain)
    end
